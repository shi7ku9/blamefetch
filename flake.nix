{
  description = "blamefetch — satirical commit co-author fetcher";
    ...
    flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { pkgs, final, ... }:
        ...
        in {
          ...
          checks = {
            build = pkgs.runCommand "blamefetch-build"
            ...
            test = pkgs.runCommand "blamefetch-test"
              ...
              mkdir $out
            '';
            clippy = pkgs.runCommand "blamefetch-clippy"
              ...
              mkdir $out
            '';
            fmt = pkgs.runCommand "blamefetch-fmt"
              { nativeBuildInputs = [ toolchain ]; } ''
              cp -r ${src} src
              chmod -R +w src
              cd src
